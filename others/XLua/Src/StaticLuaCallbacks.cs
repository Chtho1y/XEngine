    ...
    public partial class StaticLuaCallbacks
    {
        ...
        internal static int LoadFromStreamingAssetsPath(RealStatePtr L)
        {
            try
            {
                ...
                UnityEngine.Networking.UnityWebRequest req = UnityEngine.Networking.UnityWebRequest.Get(filepath);
                req.SendWebRequest();
                while (true)
                {
                    if (req.isDone || !string.IsNullOrEmpty(req.error))
                    {
                        ...
                        else
                        {
                            ...
                            if (LuaAPI.xluaL_loadbuffer(L, req.downloadHandler.data, req.downloadHandler.data.Length , "@" + filename) != 0)
                            {
